...
def gen_notes_chords(model, seed, char2idx, idx2char, vocab_size, num_chars):
    preprocessor = MidiPreprocessor("dupa")

    model.eval()
    state = None
    inputs = preprocessor.prepare_sequence_chords(seed, char2idx, onehot=False) # if error change to prepare_sequence_old
    ...
    for t in range(num_chars):
        if len(intersection(preds[-4:], preds[-8:-4])) / len(preds[-4:]) >= 0.75:
            ...
            curr_pred = curr_pred
        # print(curr_pred)
        curr_pred = preprocessor.prepare_sequence_chords([curr_pred], char2idx, onehot=False)
...
def gen_notes_two_embeddings(model_two_embeddings, num_chars):
    preprocessor = MidiPreprocessor("dupa")
    vel2idx, idx2vel, vel_VOCAB_SIZE = preprocessor.load_velocity_vocab_dicts()
    char2idx_note_dur, idx2char_note_dur, note_dur_VOCAB_SIZE = preprocessor.load_notes_durations_vocab_dicts()
    char2idx_note_dur_vel, idx2char_note_dur_vel, note_dur_vel_VOCAB_SIZE = preprocessor.load_notes_duration_velocity_vocab_dicts()

    list_of_first_notes = None
    random_notes = np.random.randint(0, len([x for x in char2idx_note_dur_vel.keys()]))
    list_of_first_notes = [[x for x in char2idx_note_dur_vel.keys()][random_notes]]
    ...
    state = None
    inputs = preprocessor.prepare_sequence(seed, vel2idx, char2idx_note_dur)
    ...
    seed_pred = seed_pred.reshape(-1, note_dur_VOCAB_SIZE)
    vel_pred = vel_pred.reshape(-1, vel_VOCAB_SIZE)

    preds = seed
    curr_pred = torch.topk(seed_pred[-1, :], k=1, dim=0)[1]
    curr_pred_vel = torch.topk(vel_pred[-1, :], k=1, dim=0)[1]

    curr_pred = idx2char_note_dur[curr_pred.item()]
    curr_pred_vel = idx2vel[curr_pred_vel.item()]

    preds.append("_".join([curr_pred, curr_pred_vel]))
    for t in range(num_chars):
        if len(intersection(preds[-4:], preds[-8:-4])) / len(preds[-4:]) >= 0.75:
            # print("random1")
            random_note = np.random.randint(0, len([x for x in char2idx_note_dur_vel.keys()]))
            curr_pred = [x for x in char2idx_note_dur_vel.keys()][random_note]  # or always same random_notes
        elif len(intersection(preds[-8:], preds[-16:-8])) / len(preds[-8:]) >= 0.75:
            # print("random2")
            random_note = np.random.randint(0, len([x for x in char2idx_note_dur_vel.keys()]))
            curr_pred = [x for x in char2idx_note_dur_vel.keys()][random_note]  # or always same random_notes
            ...
            curr_pred = curr_pred
            # print(curr_pred)
        curr_pred = preprocessor.prepare_sequence(seed, vel2idx, char2idx_note_dur)
        ...
        curr_pred = curr_pred.reshape(-1, note_dur_VOCAB_SIZE)
        curr_pred_vel = curr_pred_vel.reshape(-1, vel_VOCAB_SIZE)
...
def gen_notes_chords(model, seed, num_chars):
    preprocessor = MidiPreprocessor("dupa")
    chord2idx, idx2chord, chord_VOCAB_SIZE = preprocessor.load_chords_vocab_dicts()

    model.eval()
    state = None
    inputs = preprocessor.prepare_sequence_chords(seed, chord2idx, onehot=False)
    inputs = inputs.reshape(1, len(inputs))
    seed_pred, state = model(inputs, state)
    seed_pred = seed_pred.reshape(-1, chord_VOCAB_SIZE)
    preds = seed
    curr_pred = torch.topk(seed_pred[-1, :], k=1, dim=0)[1]
    curr_pred = idx2chord[curr_pred.item()]
    preds.append(curr_pred)
    for t in range(num_chars):
        if len(intersection(preds[-4:], preds[-8:-4])) / len(preds[-4:]) >= 0.75:
            # print("random1")
            random_note = np.random.randint(0, len([x for x in chord2idx.keys()]))
            curr_pred = [x for x in chord2idx.keys()][random_note]  # or always same random_notes
        elif len(intersection(preds[-8:], preds[-16:-8])) / len(preds[-8:]) >= 0.75:
            # print("random2")
            random_note = np.random.randint(0, len([x for x in chord2idx.keys()]))
            curr_pred = [x for x in chord2idx.keys()][random_note]  # or always same random_notes
            ...
            curr_pred = curr_pred
        # print(curr_pred)
        curr_pred = preprocessor.prepare_sequence_chords([curr_pred], chord2idx, onehot=False)
        curr_pred = curr_pred.reshape(1, len(inputs))
        curr_pred, state = model(curr_pred, state)

        curr_pred = curr_pred.reshape(-1, chord_VOCAB_SIZE)
        curr_pred = torch.topk(curr_pred[-1, :], k=1, dim=0)[1]
        curr_pred = idx2chord[curr_pred.item()]
        preds.append(curr_pred)
...
def train_chords_model():
    ...
    preprocessor = MidiPreprocessor("dupa")
    dataLoader = DataLoaderHandler("dupa")

    notes_duration_note_velocity_formatted_v2, notes_duration_velocity_formatted_v2, notes_duration_formatted_v2, \
    notes_velocity_formatted_v2, velocity_list_formatted_v2, \
    chords_duration_formatted_v2 = preprocessor.load_formatted_data()
    ...
    for epoch in range(NUM_EPOCHS):
        model_chord.train()
        for x, y in zip(dataLoader.get_chord_loaders(chords_duration_formatted_v2)):
            ...
            y_ = y_.reshape(-1, chord_VOCAB_SIZE)  # >> only when using batches
        ...
        timer_end = timer()
        PATH = "drive/MyDrive/UPC-Project/weights/fake_chord_model_{}.pt".format(round(float(loss), 1))
        torch.save(model_chord.state_dict(), PATH)
        if (epoch + 1) % 5 == 0:
...
def train_model():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for x, y in zip(dataLoader.get_note_velocity_loaders(notes_duration_velocity_formatted_v2)):
            # for x,y in zip(dataX,dataY):
            x1 = x[:, 0].to(device)
            x2 = x[:, 1].to(device)
            y1 = y[:, 0].to(device)
            ...
            y_2 = y_2.reshape(-1, vel_VOCAB_SIZE)  # >> only when using batches
        ...
        timer_end = timer()
        PATH = "drive/MyDrive/UPC-Project/weights/fake_test_two_embeddings_2_{}.pt".format(round(float(loss), 1))
        torch.save(model_two_embeddings.state_dict(), PATH)
        if (epoch + 1) % 5 == 0:
            # Generate a seed sentence to play around
